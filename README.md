...
- `"font_scale": 0.85` を付けると、そのテキストボックスの文字の大きさをすべて 0.85 倍にする。
  `{"28": 32, "36": 40}` のように書くと、その大きさ（pt）の文字だけ置き換える（本文 28→32、見出し 36→40 など）。
- **矢印の追加**: `{"type": "arrow", "from": [x, y], "to": [x, y], "color": "FFFF00", "width_pt": 6}`（cm）。
  `to` の側に三角の矢じりが付く。
- **PowerPoint で手直ししたら JSON に写す**: 生成した pptx を直接直しても、次に build するとその変更は消える。
  位置・大きさ（`box`）、文字、文字の大きさ（`font_scale`）、追加した矢印（`arrow`）を JSON に書き戻しておく。
- JSON から消した要素は、もとの pptx のまま残る（もとの画像を残したいときは、その要素を消す）。
- **要素の削除**: `{"id": <id>, "type": "delete"}` でその図形を消す。グループの id なら中身ごと消える。
  `copy_from` の複製元に使う図形を消すときは、削除を JSON の後ろに書く（上から順に処理する）。
- **グループの移動**: `{"id": <グループの id>, "type": "group", "box": [...]}` でグループごと移動・リサイズする。
- **表の追加**: `{"type": "table", "box": [...], "rows": [[見出し...], [...]], "font_size": 28, "col_widths": [比...]}`。
  1 行目は灰色の見出し。`null` のセルは上のセルと縦に結合する（群名など）。罫線だけの表スタイルで作る。
